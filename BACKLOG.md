...
- **Claude Code 的客製化介面** — Subagent(2025-07)、Hook(2025-09)、Plugin(2025-10)——
  跟 `agent-skills` 查證時發現這三個刻意設計成互斥、佔據不同判斷軸(主線程可見 vs context
  隔離 vs 不看 LLM 判斷的固定觸發),不同源,不是同一套機制的變體。份量夠大,值得單獨開一個
  主題,不要因為都跟 Skills 同期出現就順手塞進 `agent-skills` 底下。官方來源已知:
  `claude.com/blog/steering-claude-code-...`、`code.claude.com/docs/en/sub-agents`、
